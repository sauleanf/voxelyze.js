...
public:
...
#ifdef WIN32
	bool IsValid() const {return !_isnan((double)x) && _finite((double)x) && !_isnan((double)y) && _finite((double)y) && !_isnan((double)z) && _finite((double)z);}
#else
	bool IsValid() const {return !__isnand(x) && finite(x) && !__isnand(y) && finite(y) && !__isnand(z) && finite(z);}
#endif

	template <typename U> Vec3D<T>(const Vec3D<U>& s)						{x = (T)s.x; y = (T)s.y; z = (T)s.z;}
	template <typename U> operator Vec3D<U>() const							{return Vec3D<U>(x, y, z);}
	template <typename U> Vec3D<T> operator=(const Vec3D<U>& s)				{x = s.x; y = s.y; z = s.z; return *this; }
	...
	template <typename U> const friend Vec3D<T> operator*(const U f, const Vec3D<T>& v) {return v*f;}
	template <typename U> const Vec3D<T>& operator+=(const Vec3D<U>& s)		{x += s.x; y += s.y; z += s.z; return *this;}
	template <typename U> const Vec3D<T>& operator-=(const Vec3D<U>& s)		{x -= s.x; y -= s.y; z -= s.z; return *this;}
	...
	const Vec3D operator-(const Vec3D &v) const		{return Vec3D(x-v.x, y-v.y, z-v.z);}
	const Vec3D operator-() const					{return Vec3D(-x, -y, -z);}
	const Vec3D operator*(const T &f) const			{return Vec3D(f*x, f*y, f*z);}
	const friend Vec3D operator*(const T f, const Vec3D& v) {return v*f;}
	...
	bool operator!=(const Vec3D& v)					{return !(*this==v);}
	const Vec3D& operator+=(const Vec3D& s)			{x += s.x; y += s.y; z += s.z; return *this;}
	const Vec3D& operator-=(const Vec3D& s)			{x -= s.x; y -= s.y; z -= s.z; return *this;}
	const Vec3D& operator*=(const T f)				{x *= f; y *= f; z *= f; return *this;}
	const Vec3D& operator/=(const T f)				{T Inv = (T)1.0/f; x *= Inv; y *= Inv; z *= Inv; return *this;}
