...
version.
Voxelyze is distributed in the hope that it will be useful, but WITHOUT ANY
...
CVX_Voxel::CVX_Voxel(CVX_MaterialVoxel *material, short indexX, short indexY,
                     short indexZ) {
  for (int i = 0; i < 6; i++)
    links[i] = NULL;
  mat = material;
...
void CVX_Voxel::replaceMaterial(CVX_MaterialVoxel *newMaterial) {
  if (newMaterial != NULL) {
    ...
    linMom *= newMaterial->_mass / mat->_mass; // adjust momentums to keep
                                               // velocity constant across
                                               // material change
...
Vec3D<float> CVX_Voxel::externalForce() {
  ...
  if (ext->isFixed(X_TRANSLATE) || ext->isFixed(Y_TRANSLATE) ||
      ext->isFixed(Z_TRANSLATE)) {
...
Vec3D<float> CVX_Voxel::externalMoment() {
  ...
  if (ext->isFixed(X_ROTATE) || ext->isFixed(Y_ROTATE) ||
      ext->isFixed(Z_ROTATE)) {
...
// http://klas-physics.googlecode.com/svn/trunk/src/general/Integrator.cpp
// (reference)
void CVX_Voxel::timeStep(float dt) {
  ...
  if (isFloorEnabled())
    floorForce(dt, &curForce); // floor force needs dt to calculate threshold to
                               // "stop" a slow voxel into static friction.

  fricForce = curForce - fricForce;

  assert(!(curForce.x != curForce.x) || !(curForce.y != curForce.y) ||
         !(curForce.z != curForce.z)); // assert non QNAN
  linMom += curForce * dt;

  Vec3D<double> translate(
      linMom * (dt * mat->_massInverse)); // movement of the voxel this timestep

  //	we need to check for friction conditions here (after calculating the
  //translation) and stop things accordingly
  if (isFloorEnabled() &&
      floorPenetration() >= 0) { // we must catch a slowing voxel here since it
                                 // all boils down to needing access to the dt
                                 // of this timestep.
    double work =
        fricForce.x * translate.x + fricForce.y * translate.y; // F dot disp
    double hKe = 0.5 * mat->_massInverse *
                 (linMom.x * linMom.x +
                  linMom.y * linMom.y); // horizontal kinetic energy

    if (hKe + work <= 0)
      setFloorStaticFriction(true); // this checks for a change of direction
                                    // according to the work-energy principle

    if (isFloorStaticFriction()) { // if we're in a state of static friction,
                                   // zero out all horizontal motion
      linMom.x = linMom.y = 0;
  ...
  orient = Quat3D<>(angMom * (dt * mat->_momentInertiaInverse)) *
           orient; // update the orientation
...
Vec3D<double> CVX_Voxel::force() {
  ...
  for (int i = 0; i < 6; i++) {
    if (links[i])
      totalForce +=
          links[i]->force(isNegative((linkDirection)i)); // total force in LCS
  }
  totalForce =
      orient.RotateVec3D(totalForce); // from local to global coordinates
  ...
  totalForce -=
      velocity() * mat->globalDampingTranslateC(); // global damping f-cv
  totalForce.z += mat->gravityForce(); // gravity, according to f=mg

  if (isCollisionsEnabled()) {
    for (std::vector<CVX_Collision *>::iterator it = colWatch->begin();
         it != colWatch->end(); it++) {
...
Vec3D<double> CVX_Voxel::moment() {
  ...
  for (int i = 0; i < 6; i++) {
    if (links[i])
      totalMoment +=
          links[i]->moment(isNegative((linkDirection)i)); // total force in LCS
  }
  ...
  totalMoment -=
      angularVelocity() * mat->globalDampingRotateC(); // global damping
  return totalMoment;
}
...
void CVX_Voxel::floorForce(float dt, Vec3D<double> *pTotalForce) {
  ...
  if (CurPenetration >= 0) {
    ...
    pTotalForce->z +=
        normalForce -
        mat->collisionDampingTranslateC() *
            vel.z; // in the z direction: k*x-C*v - spring and damping

    if (isFloorStaticFriction()) { // If this voxel is currently in static
                                   // friction mode (no lateral motion)
      assert(horizontalVel.Length2() == 0);
      float surfaceForceSq =
          (float)(pTotalForce->x * pTotalForce->x +
                  pTotalForce->y *
                      pTotalForce->y); // use squares to avoid a square root
      float frictionForceSq =
          (mat->muStatic * normalForce) * (mat->muStatic * normalForce);

      if (surfaceForceSq > frictionForceSq)
        setFloorStaticFriction(false); // if we're breaking static friction,
                                       // leave the forces as they currently
                                       // have been calculated to initiate
                                       // motion this time step
    } else { // even if we just transitioned don't process here or else with a
             // complete lack of momentum it'll just go back to static friction
      *pTotalForce -=
          mat->muKinetic * normalForce *
          horizontalVel.Normalized(); // add a friction force opposing velocity
...
Vec3D<float> CVX_Voxel::strain(bool poissonsStrain) const {
  // if no connections in the positive and negative directions of a particular
  // axis, strain is zero
  // if one connection in positive or negative direction of a particular axis,
  // strain is that strain - ?? and force or constraint?
  // if connections in both the positive and negative directions of a particular
  // axis, strain is the average.

  Vec3D<float> intStrRet(
      0, 0,
      0); // intermediate strain return value. axes according to linkAxis enum
  int numBondAxis[3] = {0}; // number of bonds in this axis (0,1,2). axes
                            // according to linkAxis enum
...
void CVX_Voxel::enableCollisions(bool enabled, float watchRadius) {
  ...
  enabled ? boolStates |= COLLISIONS_ENABLED : boolStates &=
                                               ~COLLISIONS_ENABLED;
}
...
void CVX_Voxel::generateNearby(int linkDepth, bool surfaceOnly) {
  ...
  for (int k = 0; k < linkDepth; k++) {
    ...
    while (iCurrent != iPassEnd) {
      ...
      for (int i = 0; i < 6; i++) {
        ...
        if (pV2 &&
            std::find(allNearby.begin(), allNearby.end(), pV2) ==
                allNearby.end())
  ...
  nearby->clear();
  for (std::vector<CVX_Voxel*>::iterator it = allNearby.begin();
       it != allNearby.end(); it++) {
    CVX_Voxel*pV = (*it);
